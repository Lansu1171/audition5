...
def schemaName_follow(request, tableName, columnName):
    '''
    根据 option 字段值获取某表的单行记录接口
    组合 columnName 和 columnValue 成 dict，传入查询方法
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, 'data': []}
        # 组合查询参数
...
def schemaName_tablename_remind_columnname_type(request, tableName, columnName, type):
    '''
    后台提醒接口，判断 authSeparate 和 authTable 的权限
    '''
    if request.method in ["POST", "GET"]:
...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider${spiderSchemaName} && scrapy crawl "+tableName+"Spider -a databaseName=freetrial"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=freetrial"
        os.system(cmd)
